...
The clients above spawn the server locally over **stdio**. ChatGPT can't spawn a local process — it connects to a **remote MCP server over HTTPS**. The package ships a second entrypoint, `vtop-mcp-http`, that serves the exact same 14 tools over the MCP **Streamable HTTP** transport so you can add it as a custom ChatGPT connector.
...
3. Save. ChatGPT lists the 14 tools. In a chat, enable the connector and ask *"What's my attendance?"* — it'll call `get_captcha` → `login` → `get_attendance`.
...
├── server.ts             registers all 14 tools
├── services/
